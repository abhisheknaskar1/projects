...
use App\Services\SanctionList\Adapters\Transformers\Unsc;
use Illuminate\Console\Command;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Storage;
use XMLReader;

class UnscTransform extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:unsc-transform';
     ...
     * @var string
     */
    protected $description = 'Transform United Nations Security Council Consolidated List XML into Loadable JSON';

    public function handle(): int
    {
        $startTime = Carbon::now();
        $assetPath = 'sanction_lists/input/un-sc-consolidated.xml';
        if (Storage::disk('local')->exists($assetPath)) {
            ...
            if ($reader === false) {
                ...
                $destination = 'sanction_lists/output/un-sc-consolidated.json';
                /** @var \App\Models\SanctionList $sanctionList */
                $sanctionList = SanctionList::query()->where('code', '=', SanctionListCode::UN_CONSOLIDATED_LIST->value)->firstOrFail();
                $vendorSanctionList = Unsc::transform($sanctionList, $reader);
                $reader->close();
                if (
