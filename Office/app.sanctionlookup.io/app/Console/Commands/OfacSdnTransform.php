...
use App\Services\SanctionList\Adapters\Transformers\OfacSdn;
use Illuminate\Console\Command;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Storage;
use XMLReader;

class OfacSdnTransform extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:ofac-sdn-transform';
     ...
     * @var string
     */
    protected $description = 'Transform OFAC SDN List XML into Loadable JSON';

    public function handle(): int
    {
        $startTime = Carbon::now();
        $assetPath = 'sanction_lists/input/ofac-sdn.xml';
        if (Storage::disk('local')->exists($assetPath)) {
            ...
            if ($reader === false) {
                ...
                $destination = 'sanction_lists/output/ofac-sdn.json';
                ...
                $vendorSanctionList = OfacSdn::transform($sanctionList, $reader);
                $reader->close();
                if (
